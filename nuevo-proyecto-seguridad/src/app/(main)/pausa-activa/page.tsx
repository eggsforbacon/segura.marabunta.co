...
const getDailyRoutine = (allExercises: Exercise[], dayOfWeek: number): Exercise[] => {
    const shuffled = [...allExercises].sort((a, b) => (a.title.charCodeAt(0) % (dayOfWeek + 1)) - (b.title.charCodeAt(0) % (dayOfWeek + 1)));
...
const SelectionCard: FC<SelectionCardProps> = ({ title, description, icon, onClick, gradient }) => (
    <button onClick={onClick} className="w-full md:w-2/5 lg:w-1/3 bg-gray-800 p-8 rounded-2xl shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 ease-in-out text-center group relative overflow-hidden">
        <div className={`absolute top-0 left-0 w-full h-1 ${gradient}`}></div>
        <div className="flex justify-center mb-6">{icon}</div>
        <h2 className="text-3xl font-bold text-white mb-2">{title}</h2>
        <p className="text-gray-400">{description}</p>
        <div className="mt-6"><span className={`inline-block ${gradient} text-white font-semibold py-2 px-6 rounded-lg transition-colors`}>Ver Ejercicios</span></div>
    </button>
);
...
export default function PausasActivasPage() {
    ...
    useEffect(() => {
        ...
        if (activeType) {
            const exercises = activeType === 'administrative' ? administrativeExercises : operationalExercises;
                ...
                {!activeType ? (
                    ...
                    <div>
                        ...
                        <div className="bg-gray-800/50 backdrop-blur-sm border  border-gray-700 p-6 rounded-2xl shadow-lg mb-12">
                            <div className="flex items-center mb-4"><Calendar className="h-8 w-8 text-purple-400"/><div className="ml-3"><h2 className="text-2xl font-bold text-white">Tu Rutina de 5 Minutos para Hoy</h2><p className="text-gray-400 font-medium">Recomendaciones para el día de {dayName}</p></div></div>
                             <div className="flex overflow-x-auto space-x-6 pb-10  "> {dailyRoutine.map((exercise, index) => (  <ExerciseCard key={index} exercise={exercise} isCompact={true} />   ))}  </div>
                             <p className="text-gray-400 font-medium">Recuerda que una pausa de 5 minutos no es una interrupción de tu trabajo, es una inversión en tu energía y concentración para hacerlo mejor.</p>
                        </div>
                        ...
                        <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white p-6 rounded-2xl shadow-lg mb-12 flex flex-col md:flex-row justify-between items-center">
                            <div className='mb-4 md:mb-0'><h2 className="text-2xl font-bold">Check Point Semanal</h2><p>Marca tu participación en la pausa activa de hoy.</p></div>
                            <div className="flex items-center gap-4"><div className="text-center"><p className="text-4xl font-bold">{weeklyCount}</p><p className="text-sm font-semibold">Participantes esta semana</p></div><button onClick={handleCountClick} disabled={hasClicked} className={`flex items-center font-bold py-3 px-6 rounded-lg transition-all duration-300 shadow-md transform hover:scale-105 ${hasClicked ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-white text-purple-600 hover:bg-purple-50'}`}><CheckCircle className="h-6 w-6 mr-2"/>{hasClicked ? '¡Ya participaste!' : '¡Me uno a la pausa!'}</button></div>
                        </div>

                        <h2 className="text-2xl font-bold text-white mb-6">Explorar todos los ejercicios</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{allExercises.map((ex, i) => (<ExerciseCard key={i} exercise={ex} />))}</div>
